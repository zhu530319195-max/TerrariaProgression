...
info = (f"TerrariaProgression P0 0.1.0\nCommit: {commit}\n"
        "Target: tModLoader v2026.07.3.0 / Terraria 1.4.4.9 / .NET 8\n"
        f"TerrariaProgression.tmod SHA256: {hashlib.sha256(mod).hexdigest()}\n"
        "P0 only. No implemented talent effects. No CI harness in the player package.\n")
with zipfile.ZipFile(output / "TerrariaProgression_P0_0.1.0_Test.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    archive.writestr("TerrariaProgression.tmod", mod)
    archive.write(repo / "docs/P0_TEST_GUIDE_zh-CN.md", "安装与测试说明.md")
    archive.writestr("BUILD_INFO.txt", info)
with zipfile.ZipFile(output / "TerrariaProgression_P0_0.1.0_Source.zip", "w", zipfile.ZIP_DEFLATED) as archive:
