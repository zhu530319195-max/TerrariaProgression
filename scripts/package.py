...
core_count = re.search(r"Final P2-Afflictions core checks passed: (\d+)", core_log)
assert core_count and runtime_pass.startswith("TP_RUNTIME_PASS "), "Package requires passing core and native checks"
info = (f"TerrariaProgression P2-Afflictions 0.9.0\nCommit: {commit}\n"
        ...
        "P2-Afflictions: 51 numeric entries plus 36 functional entries (including fourteen numeric effects), 87 total. No CI harness in the player package.\n")
with zipfile.ZipFile(output / "TerrariaProgression_P2Afflictions_0.9.0_Test.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    archive.writestr("TerrariaProgression.tmod", mod)
    archive.write(repo / "docs/P2Afflictions_TEST_GUIDE_zh-CN.md", "安装与测试说明.md")
    archive.writestr("BUILD_INFO.txt", info)
with zipfile.ZipFile(output / "TerrariaProgression_P2Afflictions_0.9.0_Source.zip", "w", zipfile.ZIP_DEFLATED) as archive:
