...
core_count = re.search(r"Final P3-Gathering core checks passed: (\d+)", core_log)
assert core_count and runtime_pass.startswith("TP_RUNTIME_PASS "), "Package requires passing core and native checks"
info = (f"TerrariaProgression P3-Gathering 0.10.0\nCommit: {commit}\n"
        ...
        "P3-Gathering: 51 numeric entries plus 39 functional entries (including sixteen numeric effects), 90 total. No CI harness in the player package.\n")
with zipfile.ZipFile(output / "TerrariaProgression_P3Gathering_0.10.0_Test.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    archive.writestr("TerrariaProgression.tmod", mod)
    archive.write(repo / "docs/P3Gathering_TEST_GUIDE_zh-CN.md", "安装与测试说明.md")
    archive.writestr("BUILD_INFO.txt", info)
with zipfile.ZipFile(output / "TerrariaProgression_P3Gathering_0.10.0_Source.zip", "w", zipfile.ZIP_DEFLATED) as archive:
