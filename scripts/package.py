...
core_count = re.search(r"Final P3-NativeTerrain core checks passed: (\d+)", core_log)
assert core_count and runtime_pass.startswith("TP_RUNTIME_PASS "), "Package requires passing core and native checks"
info = (f"TerrariaProgression P3-ToolPowerBlast 0.12.1\nCommit: {commit}\n"
...
with zipfile.ZipFile(output / "TerrariaProgression_P3ToolPowerBlast_0.12.1_Test.zip", "w", zipfile.ZIP_DEFLATED) as archive:
...
with zipfile.ZipFile(output / "TerrariaProgression_P3ToolPowerBlast_0.12.1_Source.zip", "w", zipfile.ZIP_DEFLATED) as archive:
