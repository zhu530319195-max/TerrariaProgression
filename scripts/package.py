...
info = (f"TerrariaProgression P3-ResourcesLimits 0.13.0\nCommit: {commit}\n"
...
with zipfile.ZipFile(output / "TerrariaProgression_P3ResourcesLimits_0.13.0_Test.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    archive.writestr("TerrariaProgression.tmod", mod)
    archive.write(repo / "docs/P3ResourcesLimits_TEST_GUIDE_zh-CN.md", "安装与测试说明.md")
    archive.writestr("BUILD_INFO.txt", info)
with zipfile.ZipFile(output / "TerrariaProgression_P3ResourcesLimits_0.13.0_Source.zip", "w", zipfile.ZIP_DEFLATED) as archive:
