...
core_count = re.search(r"Final Loadouts core checks passed: (\d+)", core_log)
...
info = (f"{build['displayName']} {version}\nInternal name: TerrariaProgression\nCommit: {commit}\n"
        ...
        "Loadouts v4 / protocol18: 52 numeric entries plus 48 functional entries, 100 total. No CI harness in the player package.\n")
with zipfile.ZipFile(output / f"BoundlessPotential_{version}_Test.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    ...
    archive.write(repo / "docs/TALENT_LOADOUTS_TEST_GUIDE_zh-CN.md", "安装与测试说明.md")
    archive.write(repo / "docs/SHARED_XP_TEST_GUIDE_zh-CN.md", "全服经验共享说明_0.14.0.md")
