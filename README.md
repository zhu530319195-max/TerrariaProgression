...
当前阶段：**P2-C 0.6.0 异常免疫与战斗触发测试版**。P2-B 已通过用户验收，PR #7 已合并。新增异常状态免疫组合、星落反击、蜂群反击、受击加速、攻击燃烧和攻击中毒，总目录 71 项。

- 按 P 或 `/tptalents` 打开页面。免疫组合在“功能能力”，其余五项在“战斗”。
- 免疫组合与受击加速各 2 点解锁；两种反击和两种攻击异常每级 1 点，可调当前强度。
- 反击伤害为原生基础伤害 × 当前等级，弹体数量固定为原生范围；攻击异常每级持续 2 秒，原生持续伤害不变。
- 存档 v3 兼容 v1/v2；升级前备份角色，退回旧 MOD 需恢复旧备份。
- 多人协议 8，服务器与客户端都需更新至 0.6.0。
- [P2-C 中文安装与测试](docs/P2C_TEST_GUIDE_zh-CN.md)
- [P2-C 实现、验证与限制](docs/P2C_IMPLEMENTATION.md)
- [P2-B 中文安装与测试（历史）](docs/P2B_TEST_GUIDE_zh-CN.md)
