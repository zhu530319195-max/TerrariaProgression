...
用户确认每套方案都可独立分配角色累计获得的全部点数，只有当前方案生效。新建、复制、重命名、删除和免费切换；旧加点迁移为第一页。经验与总点数共有，每页独立保存等级、强度、开关、子效果与真实投入账本；退款不增加角色总额度。切换不恢复生命魔力或重置冷却，停止旧采集作业。服务器上限保持。实现详见docs/TALENT_LOADOUTS_zh-CN.md，升级为存档v4／协议18；本批未授权合并。

## D064 — 改名输入修复与自定义方案组合键

2026-09-18：用户报告改名可退格但不能输入，要求自定义快捷键，例如Shift+1选择方案1。继续未合并PR #18，修订版0.15.1，不合并。文字输入按原生绘制阶段处理，输入焦点覆盖原版重置，保留中文输入、关闭清理与输入时禁用快捷键。

本机快捷键设置默认Shift+1～9、Shift+0选择列表第1～10套，左右Shift均可；可配置单键、Shift/Ctrl/Alt组合键及对应方案序号，也可清空或总关闭。按主键的按下沿触发一次，按住不连发；按序号选择现有方案，不创建方案，删除后后续序号前移。名称改动不影响顺序，面板显示序号。重名仍按内部ID区分，发送既有服务端切换意图，等待回复时不预先改状态。输入文字、聊天、牌子／箱子、暂停／设置、失焦或死亡时不切换。重复组合优先列表第一条，每次最多一个请求；数字组合被识别时抑制同键原版快捷栏选择。存档v4／协议18不变，角色点数和切换限制不变。
